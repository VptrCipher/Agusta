...
function CommentItem({
  ...
  return (
    <List.Item
      style={{ borderBlockEnd: '1px solid #2b2825', alignItems: 'flex-start', display: 'block' }}
      ...
      <List.Item.Meta
        ...
        title={(
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
            ...
            <Tooltip title={new Date(comment.created_at).toLocaleString()}>
              <span style={{ ...typography.compact, ...tabularNumbersStyle, color: 'rgba(240, 238, 231, 0.35)', whiteSpace: 'nowrap' }}>
        ...
        description={(
          <div style={{ display: 'grid', gap: 8 }}>
            {comment.body && <div style={{ ...typography.body, whiteSpace: 'pre-wrap', color: 'rgba(240, 238, 231, 0.75)' }}>{comment.body}</div>}
...
export default function DiscussionThread({ contentType, objectId }: DiscussionThreadProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '0 4px 24px' }}>
      ...
      {replyTo && (
        <div style={{ color: 'rgba(240, 238, 231, 0.45)', margin: '8px 0' }}>
