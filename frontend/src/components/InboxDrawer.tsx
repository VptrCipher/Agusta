...
function ReplyContext({ row }: { row: InboxMessage }) {
  ...
  return (
    <>
      <span style={{ color: 'rgba(240, 238, 231, 0.45)' }}> replied to </span>
      <span>{parentLabel(row)}</span>
      <Tooltip title={row.parent_body || 'No text content'}>
        <MessageOutlined style={{ color: 'rgba(240, 238, 231, 0.45)', fontSize: typography.compact.fontSize, marginInlineStart: 4 }} />
      </Tooltip>
    </>
  )
}
...
function RecordLink({ message: row, onOpenResource }: { message: InboxMessage; onOpenResource?: (resourceKey: string, rowId: string | number) => void }) {
  ...
  return (
    <div style={{ ...typography.compact, marginTop: 6, color: 'rgba(240, 238, 231, 0.68)', minWidth: 0 }}>
      ...
      {canOpen ? (
        ...
        <span style={{ color: 'rgba(240, 238, 231, 0.88)' }}>{label}</span>
      )}
    </div>
  )
}
    ...
    <>
      <Badge count={unreadCount} size="small" offset={[-2, 5]}>
        <Button
          ...
          style={{ color: 'rgba(240, 238, 231, 0.85)' }}
      ...
      >
        <div style={{ padding: 12, borderBottom: '1px solid #35322e' }}>
                ...
                renderItem={(row) => (
                  <List.Item
                    ...
                    style={{
                      ...
                      minWidth: 0,
                      overflow: 'hidden',
                      borderInlineStart: selected?.id === row.id ? '3px solid #d97757' : '3px solid transparent',
                      background: row.is_read ? 'transparent' : 'rgba(217, 119, 87, 0.08)',
                    }}
                  >
                    <List.Item.Meta
                      avatar={(
                      ...
                      description={(
                        <div style={{ display: 'grid', gap: 8 }}>
                          {selected?.id === row.id && selected ? (
                            ...
                            <>
                              <Typography.Text ellipsis style={{ color: 'rgba(240, 238, 231, 0.65)' }}>
                                {messagePreview(row)}
                              </Typography.Text>
                              <RecordLink message={row} onOpenResource={onOpenResource} />
                            </>
        ...
        {replyTo && (
          <div style={{ color: 'rgba(240, 238, 231, 0.45)', padding: '8px 12px 0' }}>
            Replying to {senderLabel(replyTo)}
            <Button type="link" size="small" onClick={() => setReplyTo(null)}>Cancel</Button>
          </div>
        )}
        <div style={{ padding: 12, borderTop: '1px solid #35322e' }}>
          <MessageComposer
            compact
