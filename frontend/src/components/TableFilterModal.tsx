          ...
          tagRender={({ label, value, closable, onClose }) => (
            <span
              style={{
                ...
                gap: 4,
                marginInlineEnd: 8,
                color: `var(--ant-color-${Math.abs(String(value).length % 10) + 1}, #d97757)`,
              }}
            >
              {label}
              {closable && (
                <CloseOutlined
                  style={{ fontSize: 10, color: 'rgba(240, 238, 231, 0.45)' }}
    ...
    <Space vertical size={18} style={{ width: '100%', height: '100%' }}>
      <Space vertical style={{ width: '100%' }} size={16}>
        {draftAdvanced.map((condition, index) => {
          ...
          return (
            <div key={condition.id} style={{ borderTop: index === 0 ? undefined : '1px solid #35322e', paddingTop: index === 0 ? 0 : 12 }}>
      ...
      <div style={{ marginTop: 'auto', borderTop: '1px solid #35322e', paddingTop: 12 }}>
      ...
      {savedFilters.map((item) => (
        <div
          ...
          style={{
            ...
            borderRadius: 8,
            padding: '8px 10px',
            background: loadedSavedFilterId === item.id ? 'rgba(217, 119, 87, 0.12)' : undefined,
          }}
        >
  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: `${CURRENT_FILTER_PANEL_WIDTH}px ${SAVED_FILTER_PANEL_WIDTH}px`, gap: 20, minHeight: 420 }}>
        ...
        <section style={{ borderLeft: '1px solid #35322e', paddingLeft: 16, minWidth: 0 }}>
