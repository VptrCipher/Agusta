...
function severityColor(severity: string) {
  return severityColors[severity] || '#d97757'
}

function getRiskPostureLevel(value: number | null | undefined): RiskPostureLevel {
  const score = value ?? 0
  if (score >= 80) {
    return { band: 'CRITICAL', color: '#c0453b', summary: 'Critical pressure requires command attention.' }
  }
  if (score >= 55) {
    return { band: 'ELEVATED', color: '#d9a344', summary: 'Elevated risk pressure across active signals.' }
  }
  if (score >= 35) {
    return { band: 'GUARDED', color: '#c8a35c', summary: 'Guarded posture with monitored signal pressure.' }
  }
  return { band: 'STABLE', color: '#85a96f', summary: 'Stable posture with no dominant pressure.' }
}
...
function eventAccentColor(item: DashboardHighlight) {
  ...
  if (item.status === 'Resolved' || item.status === 'Closed') return '#85a96f'
  return '#c4a2d9'
}
...
function MetricCard({
  ...
  return (
    <Card className={`dashboard-metric-card tone-${tone}${priority ? ' priority' : ''}`} style={{ '--metric-color': accent } as CSSProperties} styles={{ body: { padding: 16 } }}>
      ...
      <Statistic title={statisticTitle} value={value as string | number} suffix={suffix} styles={{ content: { color: '#f0eee7' } }} />
...
function RiskPostureCard({ data, loading }: { data: DashboardOverview | null; loading: boolean }) {
  ...
  const gaugeConfig = useMemo(() => ({
    ...
    scale: {
      color: {
        range: ['#85a96f', '#85a96f', '#d9a344', '#c0453b'],
      },
    },
    style: {
      arcLineWidth: 9,
...
function statusColor(status: string) {
  ...
  return colors[status] || '#d97757'
}

function signalColor(index: number) {
  return ['#e0a88f', '#85a96f', '#c4a2d9', '#9ab0c4', '#c8a35c', '#b294cc'][index % 6]
}
        ...
        return (
          <div className="dashboard-severity-row" key={item.label}>
            ...
            <Progress
              ...
              railColor="rgba(240, 238, 231, 0.08)"
            />
      ...
      {visibleData.map((item) => {
        ...
        return (
          <div className="dashboard-workload-row" key={item.label}>
            ...
            <Progress percent={percent} showInfo={false} strokeColor={statusColor(item.label)} railColor="rgba(240, 238, 231, 0.08)" />
          ...
          {tactics.map((tactic) => {
            ...
            return (
              <span
                ...
                style={{
                  background: value > 0 ? `linear-gradient(135deg, ${color}${Math.round(34 + intensity * 108).toString(16).padStart(2, '0')}, rgba(240, 238, 231, 0.04))` : 'rgba(240, 238, 231, 0.035)',
                  borderColor: value > 0 ? `${color}66` : 'rgba(240, 238, 231, 0.06)',
                  color: value > 0 ? '#f0eee7' : 'rgba(240, 238, 231, 0.24)',
  ...
  return (
    <div className="dashboard-mini-list">
      {data.map((item) => {
        ...
        const color = item.label === 'Failed' ? '#c0453b' : item.label === 'Success' ? '#85a96f' : item.label === 'Running' ? '#d97757' : '#d9a344'
        return (
          <div className="dashboard-progress-row" key={item.label}>
            ...
            <Progress percent={percent} showInfo={false} strokeColor={color} railColor="rgba(240, 238, 231, 0.08)" />
          </div>
        )
      })}
    </div>
...
function CoveragePanel({ data }: { data: DashboardOverview }) {
  const rows = [
    { label: 'Enrichment coverage', value: data.coverage.enrichment_coverage, color: '#85a96f' },
    { label: 'Playbook coverage', value: data.coverage.playbook_coverage, color: '#d97757' },
  ]
    ...
    <div className="dashboard-mini-list">
      {rows.map((row) => (
        <div className="dashboard-progress-row" key={row.label}>
          ...
          <Progress percent={row.value || 0} showInfo={false} strokeColor={row.color} railColor="rgba(240, 238, 231, 0.08)" />
        </div>
      ))}
      ...
      <div className="dashboard-shell">
        <section className="dashboard-hero">
          ...
          <div className="dashboard-command-strip">
            ...
            <CommandSignalPill label="Critical Cases" value={formatNumber(summary?.open_critical_cases)} color="#c0453b" />
            <CommandSignalPill label="Critical/High Alerts" value={formatNumber(summary?.critical_high_alerts)} color="#d9a344" />
            <CommandSignalPill label="Automation" value={formatPercent(summary?.automation_success_rate)} color={summary?.failed_playbooks ? '#c0453b' : '#85a96f'} />
            <CommandSignalPill label="Window" value={selectedWindow.toUpperCase()} color="#e0a88f" />
          </div>
        </section>
        ...
        <div className="dashboard-posture-grid">
          ...
          <div className="dashboard-posture-stack">
            <div className="dashboard-grid posture-kpis">
              <MetricCard title="Open Critical Cases" value={formatNumber(summary?.open_critical_cases)} accent="#c0453b" category="Primary Risk" tone="critical" priority sub={`${formatNumber(summary?.open_cases)} open cases`} />
              <MetricCard title="Critical / High Alerts" value={formatNumber(summary?.critical_high_alerts)} accent="#d9a344" category="Primary Risk" tone="warning" priority sub={`${formatNumber(summary?.total_alerts)} alerts observed`} />
              <MetricCard title="Automation Success" value={formatPercent(summary?.automation_success_rate)} accent={summary?.failed_playbooks ? '#c0453b' : '#85a96f'} category="Response Capacity" tone={automationTone(summary)} sub={`${formatNumber(summary?.running_playbooks)} running · ${formatNumber(summary?.failed_playbooks)} failed`} />
              <MetricCard title="Cases In Window" value={formatNumber(summary?.total_cases)} accent="#d97757" category="Operating Volume" tone="signal" sub={`${selectedWindow} operating window`} />
              <MetricCard title="Artifacts In Scope" value={formatNumber(summary?.total_artifacts)} accent="#a98bc4" category="Entity Surface" tone="intel" sub="Distinct entities linked to alerts" />
              <MetricCard title="Knowledge Signals" value={formatNumber(summary?.total_knowledge)} accent="#85a96f" category="Intel Depth" tone="intel" sub={`${formatNumber(summary?.total_enrichments)} enrichments`} />
            </div>
