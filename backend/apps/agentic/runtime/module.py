...
def default_module_scripts_dir():
    """Official Modules that ship with AGUSTA."""
    return Path(settings.BASE_DIR) / "modules"


def default_custom_module_scripts_dir():
    """Operator-supplied Modules, mounted as a volume in a deployment."""
    return Path(settings.CUSTOM_DIR) / "modules"


def default_module_script_dirs():
    # Official first, custom last: a custom script with the same filename
    # overrides the official one. Mirrors default_playbook_script_dirs().
    return [default_module_scripts_dir(), default_custom_module_scripts_dir()]
