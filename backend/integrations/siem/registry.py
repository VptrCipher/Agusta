...
from agusta import settings

logger = logging.getLogger(__name__)

OFFICIAL_REGISTRY_DIR = Path(settings.BASE_DIR) / "data" / "siem"
CUSTOM_REGISTRY_DIR = Path(settings.CUSTOM_DIR) / "data" / "siem"


def default_registry_dirs():
    # Official first, custom last: a custom YAML with the same filename
    # overrides the official index definition.
    return [OFFICIAL_REGISTRY_DIR, CUSTOM_REGISTRY_DIR]
