...
from .api_client import AgustaClient
...
app = typer.Typer(no_args_is_help=True, invoke_without_command=True, help="AGUSTA command line client.")
auth_app = typer.Typer(no_args_is_help=True, help="Authenticate and inspect the current AGUSTA session.")
config_app = typer.Typer(no_args_is_help=True, help="Read and write AGUSTA CLI settings.")
completion_app = typer.Typer(no_args_is_help=True, help="Show shell completion installation commands.")
case_app = typer.Typer(no_args_is_help=True, help="List, show, and update AGUSTA cases.")
alert_app = typer.Typer(no_args_is_help=True, help="List and show AGUSTA alerts.")
artifact_app = typer.Typer(no_args_is_help=True, help="List and show AGUSTA artifacts.")
knowledge_app = typer.Typer(no_args_is_help=True, help="Search, show, and update AGUSTA knowledge.")
comment_app = typer.Typer(no_args_is_help=True, help="List and add AGUSTA comments.")
file_app = typer.Typer(no_args_is_help=True, help="Upload, inspect, download, and read AGUSTA files.")
enrichment_app = typer.Typer(no_args_is_help=True, help="Create AGUSTA enrichments.")
playbook_app = typer.Typer(no_args_is_help=True, help="List and run AGUSTA playbooks.")
playbook_template_app = typer.Typer(no_args_is_help=True, help="List playbook templates.")
siem_app = typer.Typer(no_args_is_help=True, help="Search and query AGUSTA SIEM integrations.")
...
def main(
    ...
    api_url: Annotated[str | None, typer.Option("--api-url", help="Temporarily override the AGUSTA base URL.")] = None,
    api_key: Annotated[str | None, typer.Option("--api-key", help="Temporarily override the AGUSTA API key.")] = None,
...
def auth_login(
    ctx: typer.Context,
    api_url: Annotated[str, typer.Option("--api-url", help="AGUSTA base URL, for example https://agusta.example.com.")],
    api_key: Annotated[str, typer.Option("--api-key", help="AGUSTA user API key.")],
    local: Annotated[bool, typer.Option("--local", help="Write .agusta/settings.json in the current directory.")] = False,
...
def auth_logout(
    ctx: typer.Context,
    local: Annotated[bool, typer.Option("--local", help="Remove auth from local .agusta/settings.json instead of global settings.")] = False,
    output: Annotated[OutputFormat | None, typer.Option("--output", help="Output format.")] = None,
) -> None:
...
def config_set(
    ...
    local: Annotated[bool, typer.Option("--local", help="Write local .agusta/settings.json instead of global settings.")] = False,
    output: Annotated[OutputFormat | None, typer.Option("--output", help="Output format.")] = None,
) -> None:
...
def _auth_login(runtime: RuntimeOptions, output: OutputFormat, api_url: str, api_key: str, local: bool) -> None:
    client = AgustaClient(api_url=api_url, api_key=api_key, verbose=runtime.verbose, console=err_console)
    ...
    if output == OutputFormat.human:
        ...
        console.print(
            key_value_table(
                "AGUSTA auth verified and saved",
                [
                    ("Scope", data["scope"]),
...
def _auth_status(runtime: RuntimeOptions, output: OutputFormat) -> None:
    ...
    client = AgustaClient(api_url=config.api_url or "", api_key=config.api_key, verbose=runtime.verbose, console=err_console)
    payload = client.version()
    data = {
...
def _auth_logout(runtime: RuntimeOptions, output: OutputFormat, local: bool) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA auth removed", list(data.items())))
        return
...
def _config_list(runtime: RuntimeOptions, output: OutputFormat) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA config", [
...
def _config_get(runtime: RuntimeOptions, output: OutputFormat, key: str) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA config value", list(data.items())))
        return
...
def _config_set(runtime: RuntimeOptions, output: OutputFormat, key: str, value: str, local: bool) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA config saved", list(data.items())))
        return
...
def _doctor(runtime: RuntimeOptions, output: OutputFormat) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA doctor", [
...
def _update_case_ai(runtime: RuntimeOptions, output: OutputFormat, case_id: str, **fields) -> None:
    ...
    _emit_agent_payload(runtime, output, "case.update_ai", payload, lambda data, _meta: _detail_table(
        "Updated AGUSTA case AI analysis",
        data,
...
def _download_file(runtime: RuntimeOptions, output: OutputFormat, file_key: str, output_path: Path | None) -> None:
    ...
    if output == OutputFormat.human:
        console.print(key_value_table("AGUSTA file downloaded", [
...
def _siem_schema_detail(data: dict) -> Table:
    table = _detail_table("AGUSTA SIEM schema", data, ["name", "backend", "description"])
...
def _require_config(runtime: RuntimeOptions):
    ...
    if missing:
        raise CliError(
            "missing_config",
            "AGUSTA API URL and API key are required. Run: agusta auth login --api-url <url> --api-key <key>",
            {"missing": missing},
