...
class AgustaClient:
    ...
    def request(self, method: str, path: str, *, authenticated: bool = True, json: Any = None, files: Any = None) -> dict[str, Any]:
        ...
        headers = {
            ...
            "User-Agent": f"agusta-cli/{__version__}",
        }
        ...
        try:
            ...
            raise CliError("network_error", f"Unable to reach AGUSTA server: {exc}", {"url": _redact_url(url)}, EXIT_NETWORK) from exc
...
def _normalize_base_url(api_url: str) -> str:
    ...
    if base.endswith("/api"):
        base = base[:-4]
    if not base:
        raise CliError("missing_api_url", "AGUSTA API URL is required", {}, EXIT_USAGE)
    return base
...
def _redact_url(url: str) -> str:
    return url.replace(redact_secret(url), "****") if "agusta_" in url else url
